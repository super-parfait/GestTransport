...
class _ClientPaymentScreenState extends State<ClientPaymentScreen> {
  ...
  static const _modes = ['Espèces', 'Virement', 'Chèque', 'Orange Money', 'Wave'];
  ...
  Map<String, dynamic>? get _clientData => _selectedClient == null ? null
      : AppData.clients.cast<Map<String, dynamic>?>()
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      appBar: AppBar(title: const Text('Règlement client'), backgroundColor: AppColors.surface),
      body: ListView(padding: const EdgeInsets.all(16), children: [
        ...
        AppSectionCard(title: 'Client', icon: Icons.person_rounded, children: [
          DropdownButtonFormField<String>(
            ...
            items: AppData.clients.map((c) => DropdownMenuItem<String>(
              value: c['name'] as String,
              child: Text(c['name'] as String),
            )).toList(),
            onChanged: (v) => setState(() { _selectedClient = v; _amountCtrl.clear(); }),
          ),
        ]),
        ...
        if (client != null) ...[
          Container(
            ...
            decoration: BoxDecoration(
              color: balance > 0 ? AppColors.errorSurface : AppColors.successSurface,
              borderRadius: BorderRadius.circular(12),
              border: Border.all(color: (balance > 0 ? AppColors.error : AppColors.success).withOpacity(0.3)),
            ),
            child: Row(children: [
              Icon(balance > 0 ? Icons.account_balance_wallet_rounded : Icons.check_circle_rounded,
                color: balance > 0 ? AppColors.error : AppColors.success, size: 22),
        ...
        // Montant & mode
        AppSectionCard(title: 'Paiement', icon: Icons.payments_rounded, children: [
          AppMoneyField(label: 'Montant réglé', controller: _amountCtrl,
            onChanged: (_) => setState(() {})),
          const SizedBox(height: 14),
          Text('Mode de paiement', style: AppTextStyles.bodyMedium.copyWith(fontWeight: FontWeight.w500)),
          const SizedBox(height: 8),
          Wrap(spacing: 8, runSpacing: 8, children: _modes.map((m) => GestureDetector(
            onTap: () => setState(() => _mode = m),
            child: AnimatedContainer(
              duration: const Duration(milliseconds: 150),
              padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 8),
              decoration: BoxDecoration(
                color: _mode == m ? AppColors.primary : AppColors.surfaceVariant,
                borderRadius: BorderRadius.circular(20),
              ),
              child: Text(m, style: AppTextStyles.bodySmall.copyWith(
                color: _mode == m ? Colors.white : AppColors.textSecondary,
                fontWeight: _mode == m ? FontWeight.w600 : FontWeight.w400,
              )),
            ),
          )).toList()),
          const SizedBox(height: 12),
          AppTextField(label: 'Note (optionnel)', controller: _noteCtrl, hint: 'Ex: Acompte semaine 25'),
        ]),
        const SizedBox(height: 14),
        ...
        if (_amount > 0 && client != null) ...[
          AppSummaryCard(
            ...
            rows: [
              ...
              AppSummaryRow(label: 'Solde avant', value: AppData.fmtMoneyFull(balance), valueColor: AppColors.error),
              AppSummaryRow(label: 'Montant réglé', value: AppData.fmtMoneyFull(_amount), valueColor: AppColors.success),
              const AppSummaryRow(label: '────────────', value: ''),
              AppSummaryRow(
                label: remaining < 0 ? '⚠️ Sur-paiement' : 'Reste à payer',
                value: AppData.fmtMoneyFull(remaining.abs()),
                valueColor: remaining < 0 ? AppColors.warning : remaining == 0 ? AppColors.success : AppColors.error,
                isBold: true,
              ),
            ],
          ),
          if (remaining < 0) ...[
            ...
            Container(
              ...
              decoration: BoxDecoration(
                color: AppColors.warningSurface, borderRadius: BorderRadius.circular(10),
        ...
        if (_saved)
          Container(
            ...
            padding: const EdgeInsets.all(14),
            decoration: BoxDecoration(
              color: AppColors.successSurface, borderRadius: BorderRadius.circular(12),
