...
class OilChangeScreen extends StatefulWidget {
  ...
  const OilChangeScreen({super.key, required this.truckPlate, this.oilChanges = const []});
...
class _OilChangeScreenState extends State<OilChangeScreen> {
  ...
  @override
  void dispose() { _kmCtrl.dispose(); _huileCtrl.dispose(); super.dispose(); }
  ...
  Widget build(BuildContext context) {
    ...
    return isStandalone
        ? Scaffold(
            ...
            appBar: AppBar(title: Text('Vidange — ${widget.truckPlate}'), backgroundColor: AppColors.surface),
            body: _buildBody(),
          )
        : _buildBody();
  }
  ...
  Widget _buildBody() => ListView(padding: const EdgeInsets.all(16), children: [
    ...
    AppSectionCard(title: 'Type de suivi', icon: Icons.tune_rounded, children: [
      Row(children: [
        Expanded(child: _typeBtn('Par km', Icons.speed_rounded)),
        const SizedBox(width: 10),
        Expanded(child: _typeBtn('Par semaines', Icons.date_range_rounded)),
      ]),
    ]),
    const SizedBox(height: 14),

    AppSectionCard(title: 'Détails', icon: Icons.oil_barrel_rounded, children: [
      AppTextField(
        label: _type == 'Par km' ? 'Kilométrage actuel' : 'Date de la vidange',
        controller: _kmCtrl,
        keyboardType: _type == 'Par km' ? TextInputType.number : TextInputType.text,
        required: true,
      ),
      const SizedBox(height: 12),
      AppTextField(label: "Type d'huile", controller: _huileCtrl,
        hint: 'Ex: 15W-40'),
      const SizedBox(height: 12),
    ...
    if (_saved)
      Container(
        ...
        child: Row(children: [
          ...
          const SizedBox(width: 10),
          Text('Vidange enregistrée !', style: AppTextStyles.titleMedium.copyWith(color: AppColors.success)),
        ]),
      ),
    ...
    AppButton(
      ...
      onPressed: () {
        setState(() { _saved = true; _kmCtrl.clear(); _huileCtrl.clear(); _filtre = false; });
  ...
  Widget _histCard(Map<String, dynamic> o) {
    return Container(
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          const Icon(Icons.oil_barrel_rounded, color: AppColors.primary, size: 18),
        ...
        if (o['km'] != null) Text('KM: ${o['km']}', style: AppTextStyles.bodySmall),
        if (o['prochain_km'] != null)
          Text('Prochain: ${o['prochain_km']} km', style: AppTextStyles.bodySmall.copyWith(color: AppColors.warning)),
        if (o['prochaine_date'] != null)
          Text('Prochain: ${o['prochaine_date']}', style: AppTextStyles.bodySmall.copyWith(color: AppColors.warning)),
        Text('Huile: ${o['huile']}', style: AppTextStyles.bodySmall),
        Text('Filtre: ${(o['filtre'] as bool) ? 'Changé' : 'Non changé'}',
          style: AppTextStyles.bodySmall.copyWith(
            color: (o['filtre'] as bool) ? AppColors.success : AppColors.textTertiary)),
      ]),
    );
  }
}
