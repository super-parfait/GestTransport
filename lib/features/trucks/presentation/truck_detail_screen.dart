...
class _TruckDetailScreenState extends State<TruckDetailScreen>
  ...
  static const _tabs = ['Résumé', 'Chargements', 'Dépenses', 'Entretiens', 'Vidanges', 'Documents', 'Recettes'];
      ...
      body: NestedScrollView(
                ...
                child: SafeArea(child: Padding(
                  ...
                  child: Row(children: [
                    ...
                    Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                      Text(truck['plate'],
                        style: AppTextStyles.headlineLarge.copyWith(color: Colors.white, letterSpacing: 2)),
                      Text(truck['driver'],
        ...
        body: TabBarView(
          ...
          children: [
            ...
            OilChangeScreen(truckPlate: truck['plate'], oilChanges: truck['oil_changes'] as List),
...
class _SummaryTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final alerts = truck['alerts'] as List;
    final statusLabel = AppData.truckStatusLabels[truck['status']] ?? truck['status'];
    final km = truck['km'] as int;
    final fmtKm = km.toString().replaceAllMapped(RegExp(r'(\d{1,3})(?=(\d{3})+(?!\d))'), (m) => '${m[1]} ');
    ...
    return ListView(padding: const EdgeInsets.all(16), children: [
      AppSectionCard(title: 'Informations camion', icon: Icons.info_outline_rounded, children: [
      ...
      AppSectionCard(title: 'Documents', icon: Icons.description_outlined, children: [
      ...
      AppSummaryCard(
        ...
        rows: [
          AppSummaryRow(label: 'Total recettes', value: AppData.fmtMoneyFull(revenues), valueColor: AppColors.success),
          AppSummaryRow(label: 'Total dépenses', value: AppData.fmtMoneyFull(expenses), valueColor: AppColors.error),
          AppSummaryRow(label: 'Bilan estimé', value: AppData.fmtMoneyFull(revenues - expenses),
        ...
        Container(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            Row(children: [
              ...
              Text('Alertes actives', style: AppTextStyles.titleLarge.copyWith(color: AppColors.error)),
            ]),
            ...
            ...alerts.map((a) => Padding(
              ...
              child: Row(children: [
                ...
                const SizedBox(width: 8),
                Text(_alertLabel(a.toString()), style: AppTextStyles.bodyMedium.copyWith(color: AppColors.error)),
              ]),
            )),
          ]),
        ),
...
class _LoadingsTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (loadings.isEmpty) {
      return const AppEmptyState(icon: Icons.inventory_2_rounded, title: 'Aucun chargement',
        ...
        return Container(
          ...
          child: Row(children: [
            ...
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              ...
              Row(children: [
                Text(l['type'], style: AppTextStyles.bodySmall.copyWith(color: AppColors.info)),
                const Text(' · ', style: TextStyle(color: AppColors.textTertiary)),
                Text(l['quantity'], style: AppTextStyles.bodySmall),
              ]),
              Text(l['date'], style: AppTextStyles.bodySmall),
            ])),
            Text(l['montant'], style: AppTextStyles.titleMedium.copyWith(color: AppColors.success)),
          ]),
        );
      },
    );
...
class _ExpensesTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (expenses.isEmpty) {
      return const AppEmptyState(icon: Icons.receipt_rounded, title: 'Aucune dépense',
        subtitle: 'Les dépenses de ce camion apparaîtront ici.');
    }
    final total = expenses.fold<double>(0, (s, e) => s + ((e['montant'] as num? ?? 0).toDouble()));
    return ListView(padding: const EdgeInsets.all(16), children: [
      Container(
        padding: const EdgeInsets.all(14),
        margin: const EdgeInsets.only(bottom: 16),
        decoration: BoxDecoration(
          color: AppColors.errorSurface, borderRadius: BorderRadius.circular(12),
        ...
        child: Row(children: [
          ...
          const Spacer(),
          Text(AppData.fmtMoneyFull(total), style: AppTextStyles.headlineSmall.copyWith(color: AppColors.error)),
        ]),
      ),
      ...expenses.map((e) {
        ...
        return Container(
          ...
          child: Row(children: [
            const Icon(Icons.receipt_rounded, color: AppColors.textSecondary, size: 20),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(exp['type'], style: AppTextStyles.titleLarge),
              Text(exp['date'], style: AppTextStyles.bodySmall),
            ])),
            Text('${exp['montant']} F', style: AppTextStyles.titleMedium.copyWith(color: AppColors.error)),
          ]),
        );
      }),
    ]);
...
class _MaintenanceTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (maintenances.isEmpty) {
      return const AppEmptyState(icon: Icons.build_rounded, title: 'Aucun entretien',
        ...
        return Container(
          ...
          child: Row(children: [
            Container(
              ...
              child: const Icon(Icons.build_rounded, color: AppColors.warning, size: 20),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Text('${m['montant']} F', style: AppTextStyles.titleMedium.copyWith(color: AppColors.warning)),
          ]),
        );
      },
    );
...
class _RevenuesTab extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (revenues.isEmpty) {
      return const AppEmptyState(icon: Icons.add_card_rounded, title: 'Aucune recette',
        ...
        return Container(
          ...
          child: Row(children: [
            Container(
              ...
              child: const Icon(Icons.add_card_rounded, color: AppColors.success, size: 20),
            ),
            const SizedBox(width: 12),
            Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
              Text(r['client'], style: AppTextStyles.titleLarge),
              Text('${r['voyages']} voyage(s)', style: AppTextStyles.bodySmall),
              Text(r['date'], style: AppTextStyles.bodySmall),
            ])),
            Text(r['montant'], style: AppTextStyles.titleMedium.copyWith(color: AppColors.success)),
          ]),
        );
      },
    );
