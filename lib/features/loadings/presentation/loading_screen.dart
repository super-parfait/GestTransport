...
class _ClientLoadingScreenState extends State<ClientLoadingScreen> {
    ...
    AppConfirmDialog.show(
      ...
      content: AppSummaryCard(
        ...
        rows: [
          ...
          if (isMaterial) AppSummaryRow(label: 'Carrière', value: _quarryCtrl.text),
          ...
          if (isMaterial) AppSummaryRow(label: 'Quantité', value: '${_quantityCtrl.text} T'),
          if (isTransport) AppSummaryRow(label: 'Voyages', value: _tripsCtrl.text),
          AppSummaryRow(label: 'Montant à payer', value: '${_fmt(_amountToPay)} F',
            valueColor: AppColors.success, isBold: true),
          AppSummaryRow(label: 'Total dépenses', value: '${_fmt(_totalExpenses)} F',
            valueColor: AppColors.error),
          AppSummaryRow(label: 'Marge nette', value: '${_fmt(_netMargin)} F',
            valueColor: _netMargin >= 0 ? AppColors.success : AppColors.error, isBold: true),
        ],
      ),
  ...
  Widget _buildTypeSelector() {
    return AppSectionCard(
      ...
      children: [
        Text('Sélectionnez le type de chargement', style: AppTextStyles.bodyMedium),
        const SizedBox(height: 12),
        Row(
          children: AppConstants.materialTypes.map((type) {
            final isSelected = _selectedType == type;
            final color = type == 'Transport' ? AppColors.warning
                : type == 'Sable' ? AppColors.info : AppColors.primary;
            final icon = type == 'Transport' ? Icons.local_shipping_rounded
                : type == 'Sable' ? Icons.grain_rounded : Icons.layers_rounded;
            return Expanded(
              child: Padding(
                padding: EdgeInsets.only(left: type == AppConstants.materialTypes.first ? 0 : 8),
                child: GestureDetector(
                  ...
                  child: AnimatedContainer(
                    ...
                    child: Column(
                      children: [
                        Icon(icon, color: isSelected ? color : AppColors.textTertiary, size: 26),
  ...
  Widget _buildSection1() {
    return AppSectionCard(
      ...
      children: [
        AppDatePicker(label: 'Date', value: _selectedDate, required: true,
          onChanged: (d) => setState(() => _selectedDate = d)),
        const SizedBox(height: 14),
        AppDropdown<String>(
          label: 'Client', required: true, hint: 'Sélectionner un client...',
          items: ['KOUAME Eric', 'BTP SERVICES', 'TOURE Construction', 'DIALLO Travaux']
            .map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(),
          onChanged: (v) { _clientCtrl.text = v ?? ''; },
        ),
        if (isMaterial) ...[
          const SizedBox(height: 14),
          AppDropdown<String>(
            ...
            items: ['Carrière KOSSOU', 'Usine ABATTA', 'Carrière DAOUKRO']
              .map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(),
            onChanged: (v) { _quarryCtrl.text = v ?? ''; },
          ),
        ],
        const SizedBox(height: 14),
        AppDropdown<String>(
          label: 'Camion', required: true, hint: 'Sélectionner un camion...',
          items: ['CI-1234-AB', 'CI-5678-CD', 'CI-9012-EF', 'CI-3456-GH']
            .map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(),
          onChanged: (v) { _truckCtrl.text = v ?? ''; },
        ),
        const SizedBox(height: 14),
        AppDropdown<String>(
          label: 'Chauffeur', required: true, hint: 'Sélectionner un chauffeur...',
          items: ['KONAN Yao', 'OUATTARA Issa', 'BAMBA Mamadou', 'COULIBALY Drissa']
            .map((c) => DropdownMenuItem(value: c, child: Text(c))).toList(),
          onChanged: (v) { _driverCtrl.text = v ?? ''; },
        ),
        const SizedBox(height: 14),
        AppTextField(label: 'Destination', hint: 'Ex: Yopougon Marché', controller: _destinationCtrl, required: true,
          validator: (v) => v == null || v.isEmpty ? AppStrings.required : null),
        const SizedBox(height: 14),
        AppTextField(label: 'Numéro de bon', hint: 'BON-2024-001', controller: _bonCtrl, required: true,
          validator: (v) => v == null || v.isEmpty ? AppStrings.required : null),
      ],
    );
  }
  ...
  Widget _buildSection2() {
    return AppSectionCard(
      ...
      children: [
        if (isMaterial) ...[
          AppTextField(
            label: 'Quantité (en tonnes)', hint: '0',
            controller: _quantityCtrl, required: true,
            keyboardType: TextInputType.number,
            onChanged: (_) => _compute(),
            validator: (v) => v == null || v.isEmpty ? AppStrings.required : null,
          ),
          const SizedBox(height: 14),
          AppMoneyField(label: 'Prix achat (par tonne)', controller: _buyPriceCtrl, required: true,
            onChanged: (_) => _compute()),
          const SizedBox(height: 14),
          AppMoneyField(label: 'Prix vente (par tonne)', controller: _sellPriceCtrl, required: true,
            onChanged: (_) => _compute()),
        ],
        if (isTransport) ...[
          AppTextField(
            label: 'Nombre de voyages', hint: '0',
            controller: _tripsCtrl, required: true,
            keyboardType: TextInputType.number,
            onChanged: (_) => _compute(),
            validator: (v) => v == null || v.isEmpty ? AppStrings.required : null,
          ),
        ...
        const SizedBox(height: 14),
        AppMoneyField(label: 'Prix transport (${isTransport ? "par voyage" : "par tonne"})',
          controller: _transportPriceCtrl, required: true, onChanged: (_) => _compute()),
        const SizedBox(height: 14),
        AppMoneyField(label: 'Dépense carburant', controller: _fuelCtrl, onChanged: (_) => _compute()),
        const SizedBox(height: 14),
        AppMoneyField(label: 'Frais de route', controller: _roadCtrl, onChanged: (_) => _compute()),
        const SizedBox(height: 14),
        AppMoneyField(label: 'Autres frais', controller: _otherFeesCtrl, onChanged: (_) => _compute()),
      ],
    );
  }
  ...
  Widget _buildSection3() {
    return AppSectionCard(
      ...
      children: [
        AppTextField(
          label: 'Observation', hint: 'Notes éventuelles...', maxLines: 3,
        ),
      ],
    );
  }
    ...
    return Container(
      ...
      decoration: BoxDecoration(
        gradient: LinearGradient(
          colors: [AppColors.primary.withOpacity(0.08), AppColors.info.withOpacity(0.08)],
        ...
        children: [
          Row(
            children: [
              ...
              Text('Calcul automatique', style: AppTextStyles.headlineSmall.copyWith(color: AppColors.primary)),
            ],
          ),
          const SizedBox(height: 12),
          const Divider(color: AppColors.divider),
          _calcRow('Montant à payer', '${_fmt(_amountToPay)} F', AppColors.info),
          _calcRow('Total dépenses', '${_fmt(_totalExpenses)} F', AppColors.error),
          const Divider(color: AppColors.divider),
          _calcRow('Marge nette', '${_fmt(_netMargin)} F',
            _netMargin >= 0 ? AppColors.success : AppColors.error, bold: true),
        ],
      ),
    );
  }
      ...
      child: Row(
        ...
        children: [
          ...
          Container(
            ...
            child: Text(value,
              style: (bold ? AppTextStyles.moneySmall : AppTextStyles.titleMedium)
